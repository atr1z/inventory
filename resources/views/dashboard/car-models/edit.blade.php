...
@section('title', 'Editar Modelo de Auto')

@section('header', 'Editar Modelo de Auto')
...
<div class="mb-6 flex justify-between items-center">
    ...
    <a href="{{ route('car-models.show', $carModel) }}" class="flex items-center text-ios-blue hover:underline">
...
<div class="rounded-xl overflow-hidden" :class="darkMode ? 'bg-ios-darkmode-card shadow-ios-dark' : 'bg-white shadow-ios'">
    <div class="p-6">
        <form action="{{ route('car-models.update', $carModel) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="mb-6">
                ...
                <select 
                    ...
                    @foreach($carBrands as $brand)
                        <option value="{{ $brand->id }}" {{ (old('car_brand_id', $carModel->car_brand_id) == $brand->id) ? 'selected' : '' }}>{{ $brand->name }}</option>
                    @endforeach
                </select>
                @error('car_brand_id')
            ...
            <div class="mb-6">
                ...
                <input type="text" 
                       ...
                       :class="darkMode ? 'bg-ios-darkmode-border border-ios-darkmode-border text-ios-darkmode-text' : 'bg-white border-gray-300 text-ios-dark'" 
                       id="model" name="model" value="{{ old('model', $carModel->model) }}" required>
                @error('model')
                    <p class="mt-1 text-xs text-ios-red">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-6">
                ...
                <input type="number" 
                       ...
                       :class="darkMode ? 'bg-ios-darkmode-border border-ios-darkmode-border text-ios-darkmode-text' : 'bg-white border-gray-300 text-ios-dark'" 
                       id="year" name="year" value="{{ old('year', $carModel->year) }}" 
                    ...
                    <label for="image" class="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed rounded-lg cursor-pointer hover:bg-gray-50 transition-colors duration-200" 
                        ...
                        @if($carModel->image)
                            <div class="flex flex-col items-center justify-center w-full h-32">
                                <img src="{{ route('files.car-models.image', $carModel->id) }}" 
...
@section('scripts')
<script>
    // Preview selected image and validate file size
    document.getElementById('image').addEventListener('change', function(e) {
        ...
        if (e.target.files.length > 0) {
            ...
            reader.onload = function(e) {
                fileLabel.innerHTML = `<div class="flex flex-col items-center justify-center w-full h-32">
